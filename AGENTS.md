...
This file provides guidance to AI coding agents (Zed, Kimi, OpenCode, Codex, and any
tool following the `AGENTS.md` convention) when working with code in this repository.
It mirrors [CLAUDE.md](CLAUDE.md); keep the two in sync.
...
This repo uses [OpenSpec](https://github.com/Fission-AI/OpenSpec) for spec-driven development. Specs live in `openspec/specs/`, in-flight change proposals in `openspec/changes/`, completed work in `openspec/changes/archive/`. Tools with OpenSpec adapters (Claude Code, Kimi, OpenCode, Codex) have generated skills/commands under `.claude/`, `.kimi/`, `.opencode/`, `.codex/` (`/opsx:*`). In any tool (including Zed), drive the same workflow with the CLI:

```bash
openspec new change "<kebab-case-name>"   # scaffold a new change proposal
openspec list [--specs]                    # list changes (or specs)
openspec show <item>                        # view a change or spec
openspec validate <item>                    # validate a change/spec
openspec status <change>                    # artifact completion status
openspec archive <change>                   # archive a completed change, update specs
openspec instructions <artifact>            # enriched authoring instructions
```

**Non-trivial features go through a change proposal before coding.** Typical loop: create a change → fill its artifacts (proposal, specs, tasks) → implement against tasks → validate → archive so `openspec/specs/` stays authoritative.
